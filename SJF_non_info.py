...
class SJFNonPreemptiveWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SJF Non Preemptive")
        self.arrival_times = []
        self.burst_times = []
    ...
    def SJFnonpreem(self, n, live, fig, timer):
        wt = [0] * n
        tat = [0] * n
        rt = [0] * n
        ...
        while complete != n:
            ...
            if live:
                ...
                timer.singleShot(self.burst_times[short] * 1000, loop.quit)
                # Start the event loop
                loop.exec_()
            self.removeRowByValue("Process " + str(short))
            ax.barh(y=1, left=t, width=self.burst_times[short], height=0.5, alpha=0.5)
            ax.text(
                t + (0.5 * self.burst_times[short]),
                1,
                s="P" + str(short),
                ha="left",
                va="center",
            ...
            complete += 1
            check = False
            t += self.burst_times[short]

            wt[short] = t - self.burst_times[short] - self.arrival_times[short]
    ...
    def calculate_SJF_non(self):
        ...
        avg_waiting_time, avg_turnaround_time = self.SJFnonpreem(
...
if __name__ == "__main__":
    ...
    sjf_non_window = SJFNonPreemptiveWindow()
    sjf_non_window.show()
    sys.exit(app.exec_())