...
class FCFSWindow(QMainWindow):
    def __init__(self):
        ...
        self.generate_button.clicked.connect(self.calculate_FCFS)
        ...
        # Add form inputs for process details
    ...
    def FCFS(self, n, live, fig, timer):
        wt = [0] * n  # waiting time
        tat = [0] * n  # turn around time
        end_time = self.burst_times[0]  # end time of the last process

        # Calculate waiting time for each process
        wt[0] = 0  # waiting time for first process is 0
        for i in range(1, n):
            if self.arrival_times[i] > end_time:
                # If the arrival time of the current process is greater than the end time of the last process
                # There is an idle time, and the waiting time of the current process is 0
                wt[i] = 0
                end_time = self.arrival_times[i] + self.burst_times[i]
            else:
                # Waiting time is end time of the last process - arrival time of current process
                wt[i] = end_time - self.arrival_times[i]
                end_time += self.burst_times[i]
            if wt[i] < 0:
                wt[i] = 0
        ...
        for i in range(n):
            if self.arrival_times[i] > t:
                t = self.arrival_times[i]
            if live:
                fig.canvas.draw()
                ...
                timer.singleShot(self.burst_times[i] * 1000, loop.quit)
                # Start the event loop
                loop.exec_()
            self.table.removeRow(0)
            ax.barh(y=1, left=t, width=self.burst_times[i], height=0.5, alpha=0.2)
            ax.text(
                t + (0.5 * self.burst_times[i]),
                1,
                s="P" + str(i),
                ha="left",
                va="center",
            ...
            t += self.burst_times[i]
        remain = self.table.rowCount()
        if remain > 0:
            for i in range(n, n + remain):
                if self.arrival_times[i] > t:
                    t = self.arrival_times[i]
                if live:
                    fig.canvas.draw()
                ...
                if self.arrival_times[i] > end_time:
                    ...
                    # Waiting time is end time of the last process - arrival time of current process
                    wt.append(end_time - self.arrival_times[i])
                    end_time += self.burst_times[i]

                if wt[-1] < 0:
                    wt[-1] = 0

                tat.append(self.burst_times[i] + wt[-1])
                ...
                # Update AWT and ATT
                AWT = total_wt / (n + remain)
                ATT = total_tat / (n + remain)
    ...
    def calculate_FCFS(self):
        ...
        paired_lists = list(zip(self.arrival_times, self.burst_times))

        # Sort the paired list
        paired_lists.sort()

        # Unzip the sorted list back into two lists
        self.arrival_times, self.burst_times = map(list, zip(*paired_lists))
        ...
        avg_turnaround_time, avg_waiting_time = self.FCFS(
            num_processes, live, self.figure, self.timer
        )
        self.canvas.draw()
        self.avg_outputs[0].setText(str(avg_turnaround_time))
        self.avg_outputs[1].setText(str(avg_waiting_time))
