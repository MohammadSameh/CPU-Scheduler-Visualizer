...
class PriorityPreemptiveWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Priority Preemptive")
        self.arrival_times = []
        self.burst_times = []
        self.priorities = []
        ...
        self.generate_button.clicked.connect(self.add_priority_preem)
    ...
    def priority_preem(self, n, live, fig, timer):
        wt = [0] * n
        tat = [0] * n
        rt = [0] * n
        ...
        while complete != n:
            ...
            for j in range(n):
                if (
                    (self.arrival_times[j] <= t)
                    and (self.priorities[j] <= prior)
                    and rt[j] > 0
                ):
            ...
            self.DecrementLastColumn("Process " + str(first))
            ax.barh(y=1, left=t, width=1, height=0.5, alpha=0.5)
            ax.text(
                t, 1, s="P" + str(first), ha="left", va="center"
            )  # Changed from 's=first' to 's=process_names[first]'
            ...
            if rt[first] == 0:
                ...
                wt[first] = fint - self.burst_times[first] - self.arrival_times[first]
    ...
    def add_priority_preem(self):
        ...
        avg_waiting_time, avg_turnaround_time = self.priority_preem(
...
if __name__ == "__main__":
    ...
    priority_preem_info = PriorityPreemptiveWindow()
